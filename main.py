...
from db_test import DataBase_test
import logging
import requests
import markdown
import time
...
database_test = DataBase_test("test")
...
@app.route('/test/GetStudentInfo', methods=['GET'])
...
def GetStudentInfo():
    if "test" in request.path:
        ...
        errcode, identity, id, name = data_test.GetStudentInfo(database_test, openid, id)
...
@app.route('/test/GetOpenid', methods=['GET'])
...
def GetOpenid():
    if "test" in request.path:
        ...
        errcode, identity, id, name, openid = data.GetOpenid(database_test, appid, secret, js_code)
...
@app.route('/test/BindOpenid', methods=['POST'])
...
def BindOpenid():
    if "test" in request.path:
        ...
        errcode, recordid, identity, id, name = data.BindOpenid(database_test, openid, id, identity_number)
...
@app.route('/test/TeacherScan', methods=['POST'])
...
def TeacherScan():
    if "test" in request.path:
        ...
        errcode, recordid = data.TeacherScan(database_test, room, _time)
...
@app.route('/test/UploadRecord', methods=['POST'])
...
def UploadRecord():
    if "test" in request.path:
        ...
        errcode, record = data.UploadRecord(database_test, openid, room, campus, row, col, _time)
        if "_id" in record:
...
@app.route('/test/ChangeStudentRecord', methods=['POST'])
...
def ChangeStudentRecord():
    if "test" in request.path:
        ...
        errcode = data.ChangeStudentRecord(database_test, recordid, exception, late, row, col, comment)
...
@app.route('/test/GetStudentHistory', methods=['GET'])
...
def GetStudentHistory():
    if "test" in request.path:
        ...
        errcode, records = data.GetStudentHistory(database_test, openid, startTime, stamp, stuid)
...
@app.route('/test/GetRoomHistory', methods=['GET'])
...
def GetRoomHistory():
    if "test" in request.path:
        ...
        errcode, record = data.GetRoomHistory(database_test, room, _time)
...
@app.route('/test/GetRoomHistoryByStamp', methods=['GET'])
...
def GetRoomHistoryByStamp():
    if "test" in request.path:
        ...
        errcode, record = data.GetRoomHistoryByStamp(database_test, room, _time)
...
@app.route('/test/GetRoomCourse', methods=['GET'])
...
def GetRoomCourse():
    if "test" in request.path:
        ...
        errcode, record = data.GetRoomCourse(database_test, room, _time)
...
@app.route('/test/GetRoomCourseByStamp', methods=['GET'])
...
def GetRoomCourseByStamp():
    if "test" in request.path:
        ...
        errcode, record = data.GetRoomCourseByStamp(database_test, room, _time)
...
@app.route('/test/GetStudentCourse', methods=['GET'])
...
def GetStudentCourse():
    if "test" in request.path:
        ...
        errcode, record = data.GetStudentCourse(database_test, stuid, _time)
...
@app.route('/test/GetStudentCourseByStamp', methods=['GET'])
...
def GetStudentCourseByStamp():
    if "test" in request.path:
        ...
        errcode, record = data.GetStudentCourseByStamp(database_test, stuid, _time)
...
@app.route('/test/GetTeacherCourse', methods=['GET'])
...
def GetTeacherCourse():
    if "test" in request.path:
        ...
        errcode, record = data.GetTeacherCourse(database_test, teaid, _time)
...
@app.route('/test/GetTeacherCourseByStamp', methods=['GET'])
...
def GetTeacherCourseByStamp():
    if "test" in request.path:
        ...
        errcode, record = data.GetTeacherCourseByStamp(database_test, teaid, _time)
        if record == None:
            res = {"errcode": errcode, "errmsg": errmsgs[errcode], "record": None}
            logging.info("Return: " + str(res))
            return jsonify(res) 
        _, scans = data.GetRoomHistoryByStamp(database_test, record["_room"], _time)
        details = {"total": [], "absent": [], "late": [], "intime": [], "exception": []}

        for rec in record["_stus"]:
            _, _, id, name = data.GetStudentInfo(database_test, None, rec)
...
@app.route('/test/GetTeacherHistory', methods=['GET'])
...
def GetTeacherHistory():
    if "test" in request.path:
        ...
        errcode, records = data.GetTeacherHistory(database_test, teaid, startTime)
...
def log():
    with open(f"{logtime}.log", "r") as f:
        text = f.read()
    return text
